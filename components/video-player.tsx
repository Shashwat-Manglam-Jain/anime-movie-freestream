...
export function VideoPlayer({ src, onTryNext, hasNextServer }: VideoPlayerProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Player container */}
      <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black ring-1 ring-black/10 dark:ring-white/10 shadow-2xl shadow-black/50">
        <iframe
          key={src}
          src={src}
      ...
      <div className="flex items-center justify-between gap-3 px-1">
        <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <svg className="h-3.5 w-3.5 text-muted-foreground/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {/* Trouble banner */}
      {showHelp && (
        <div className="rounded-xl bg-muted border border-border px-4 py-3 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-lg bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 px-4 py-1.5 text-xs font-medium text-foreground/70 transition-colors"
            >
              Reload
            </button>
          </div>
